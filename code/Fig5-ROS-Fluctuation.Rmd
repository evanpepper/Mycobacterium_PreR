...
```
```{r}
cfu.data.avg <- cfu.data %>% group_by(strain, treatment, bio.rep) %>%
  summarise(mean.e.mutant.proportion = mean(e.mutant.proportion))

avg.fold.change <- mean(cfu.data.avg %>% filter(treatment == 'plus') %>% pull(mean.e.mutant.proportion)) /
  mean(cfu.data.avg %>% filter(treatment == 'minus') %>% pull(mean.e.mutant.proportion))
```
...
freq.plot <- ggplot(cfu.data.avg, aes(x = treatment, y = mean.e.mutant.proportion, color = treatment)) +
  geom_boxplot(outlier.color = NA, alpha = 0.8) +
  geom_jitter(alpha = 0.5, width = 0.1, shape = 21, color = 'black', aes(fill = treatment)) +
...
Next, analyzing data from cumene hydroperoxide dose response assays for supplementary figure
...
```{r}
# generating figure
pdf(file = '../supplementary-figures/figure-panels/fig5-a.pdf', height = 4, width = 8)
print(growth.curves)
dev.off()
```
...
```{r}
# generating figure
pdf(file = '../supplementary-figures/figure-panels/fig5-b.pdf', height = 4, width = 8)
...
```{r}
# generating figure
pdf(file = '../supplementary-figures/figure-panels/fig6-a.pdf', height = 8, width = 10)
print(growth.curves)
dev.off()
```
...
```{r}
# generating figure
pdf(file = '../supplementary-figures/figure-panels/fig6-b.pdf', height = 8, width = 10)
print(dose.response.curves)
dev.off()
```
...
```{r}
pdf(file = '../supplementary-figures/figure-panels/fig6-c.pdf', height = 4, width = 10)

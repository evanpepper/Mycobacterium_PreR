...
for (i in 1:length(unique.colonies)) {
  ...
  growth.curve <- ggplot(data = smooth.data, aes(x = time, y = growth)) +
    ...
    annotate("text", x = (run + 10), y = (0.2 * max(smooth.data$pred.growth)),
             label = paste('max rate:\n', round(r, digits = 2), '\npixels/hour', sep = ''), color = 'red', size = 4) +
...
colony.metrics$conditions <- paste(colony.metrics$experiment, '-', colony.metrics$time.point, '-', colony.metrics$treatment, sep = '')
...
# untreated colonies are those which never saw antibiotic
# untreated conditions include "NA post minus" and "I pre minus"
untreated.data <- colony.metrics %>%
  filter(conditions %in% c('I-pre-minus', 'NA-post-minus'))
# counting the number of isolates screened with ScanLag
print(paste('In total, ', length(unique(treated.data$colony.label)), ' INH treated colonies (in liquid and agar) were screened with ScanLag', sep = ''))
print(paste('In total, ', length(unique(untreated.data$colony.label)), ' untreated control colonies (never saw INH) were screened with ScanLag', sep = ''))
...
growth <- merge(growth, clusters[, c(1, 10)])
